...
Our aim in this paper is to predict popularity of news articles before publication. Popularity is used in the sense that whether or not the article will be shared on Ekşi Sözlük which is the most popular social media establishment in Turkey, with Turkish origins.

This work makes several contributions. First, it explores the dynamics of stories in  Milliyet.com.tr. Second, it introduces the problem of predicting the popularity of a news article. Third, it provides a set of textual and semantic features that can be used to predict if a news article will be shared or not in Ekşi Sözlük before the said article is published. Fourth, it provides an evaluation of the introduced features. Fifth, an error analysis identifies possible causes for classification failure.
...
For semantic feature, we used ITU Natural Language Processing Toolkit. With this tool, we were able to find Named Entities of each word in the articles. According to pipeline we used the schema represented in Figure \ref{fig_sim}. Thus we were able to get person, organization and location counts for each news article. After that we used Random Forest Algorithm \cite{random-forest}. We had 4 numerical data as in Term Frequency of popular terms, Person Count, Organization Count and Location Count as well as one label which is popular or not. Consequently, we reimplemented GitHub user 'ironmanMa's algorithm for our purpose.
\begin{figure}[!t]
	\includegraphics[width=.5in]{schema.png}
